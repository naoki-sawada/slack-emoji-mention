...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_hyperapp__ = __webpack_require__(9);
...
const actions = {
  ...
  getUser: value => async (state, actions) => {
    try {
      const user = await Object(__WEBPACK_IMPORTED_MODULE_2__utils_api__["a" /* getUser */])(value);
      if (user) {
  ...
  putUser: value => async (state, actions) => {
    try {
      ...
      const user = await Object(__WEBPACK_IMPORTED_MODULE_2__utils_api__["b" /* putUser */])(token, JSON.parse(updater));
      if (user) {
        actions.setUser(user);
...
const InputArea = ({ name, userSettings, putUser }) => {
  ...
  const { token } = __WEBPACK_IMPORTED_MODULE_1_query_string___default.a.parse(location.search);
  return Object(__WEBPACK_IMPORTED_MODULE_0_hyperapp__["b" /* h */])(
    'div',
    { style: { margin: '2vh' }, 'class': name },
    [Object(__WEBPACK_IMPORTED_MODULE_0_hyperapp__["b" /* h */])(
      'textarea',
      { style: { fontSize: '2vh' }, id: 'textarea', rows: '10', cols: '80' },
...
const view = (state, actions) => {
  ...
  return Object(__WEBPACK_IMPORTED_MODULE_0_hyperapp__["b" /* h */])(
    ...
    [Object(__WEBPACK_IMPORTED_MODULE_0_hyperapp__["b" /* h */])(
      ...
      InputArea,
      { userSettings: state.userSettings, putUser: actions.putUser },
      []
    )]
  );
};
...
const strictUriEncode = __webpack_require__(11);
const decodeComponent = __webpack_require__(12);
...
/* harmony export (immutable) */ __webpack_exports__["b"] = putUser;
...
async function getUser(token) {
  try {
    const response = await __WEBPACK_IMPORTED_MODULE_0_axios__["get"](`/user?token=${token}`);
...
async function putUser(token, updater) {
  try {
    const response = await __WEBPACK_IMPORTED_MODULE_0_axios__["put"](`/user?token=${token}`, updater);
    return response.data;
  } catch (e) {
    throw new Error(e.message);
  }
